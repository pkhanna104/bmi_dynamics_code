...
def get_spike_kinematics(animal, day, order, history_bins, **kwargs):
    ...
    for i_t, tsk_fname in enumerate(day):
        ...
        for i, te_num in enumerate(tsk_fname):
            ...
            ### Get out the right targets for this specific readout.  
            trg_trl = []; trg_trl_pos = []; 
            ...
            if animal == 'grom':
                ...
                tmp, targ_i_all, targ_ix, trial_ix_all, cursor_pos = pa.extract_trials_all(hdf, rew_ix, 
                    drives_neurons_ix0=drives_neurons_ix0, hdf_key=key, keep_trials_sep=True,
                ...
                #trial_ord[order[i_t][i]] = trial_ix_all    
            ...
            for t in range(len(bin_spk)):
                if t in ix_mod:
                    ...
                    for tm in np.arange(history_bins, nt-history_bins):
                        
                        for tmpi, tmpii in enumerate(np.arange(tm - history_bins, tm + history_bins + 1)):
                            spk[tm-history_bins, tmpi, :] = trl[tmpii, :]

                        spk0[tm-history_bins, :] = trl[tm, :]
                        velo[tm-history_bins, :(2*history_bins)+1] = np.squeeze(np.array(cursor_vel[t][tm-history_bins:tm+history_bins+1, 0]))
                        velo[tm-history_bins, ((2*history_bins)+1):] = np.squeeze(np.array(cursor_vel[t][tm-history_bins:tm+history_bins+1, 1]))
                        
                        poso[tm-history_bins, :(2*history_bins)+1] = np.squeeze(np.array(cursor_pos[t][tm-history_bins:tm+history_bins+1, 0]))
                        poso[tm-history_bins, (2*history_bins)+1:] = np.squeeze(np.array(cursor_pos[t][tm-history_bins:tm+history_bins+1, 1]))
                        
                        pusho[tm-history_bins, :(2*history_bins)+1] = np.squeeze(np.array(decoder_all[t][tm-history_bins:tm+history_bins+1, 3]))
                        pusho[tm-history_bins, (2*history_bins)+1:] = np.squeeze(np.array(decoder_all[t][tm-history_bins:tm+history_bins+1, 5]))
    ...
    # Get trial Order: Temporally order targets: 
    #    temp_trial_order = np.vstack((temp_tune['trl']))

    # n0 = 0
    # for jj in range(len(trial_ord.keys())):
    #     mx = np.max(trial_ord[jj])+1
    #     trial_ord[jj] = trial_ord[jj]+n0
    #     ixx = np.nonzero(temp_trial_order[:, 1]==jj)[0]
    #     temp_trial_order[ixx, 0] += n0
    #     n0 += mx

    # Add them to trial_order in the right order: 
    # trial_order = []
    # for jj in range(len(trial_ord.keys())):
    #     trial_order.append(trial_ord[trl_rd[jj]])

    # trial_order = np.hstack((trial_order))
    ...
    hb2 = (2*history_bins) + 1; 

    sub_push_all = np.vstack((temp_tune['psh']))[:, [i_zero, i_zero+hb2]]
    ...
    #trl_temp = temp_trial_order[:, 0]

    accx = np.hstack((np.array([0.]), np.diff(velocity[:, 0])))
    accy = np.hstack((np.array([0.]), np.diff(velocity[:, 1])))

    ###################################
    ### MODEL with t, VEL, POS, ACC ###
    ###################################

    data = pandas.DataFrame({'velx': velocity[:, 0], 'vely': velocity[:, 1],'tsk': task_index, 'posx': position[:, 0], 'posy': position[:, 1]})
    ...
    for i in tli:
        ...
        for vl in ['vel', 'pos', 'psh']:
            for ixy, (nm, xy_ad) in enumerate(zip(['x', 'y'], [0, hb2])):
                if vl=='vel':
                ...
                tmp_dict[vl+nm+'_t'+nm0+str(np.abs(TL[i]))] = tmp
...
#### NEW PLOT FCNS ######
def sweep_alpha_all(run_alphas=True, model_set_number = 7):
    ...
    for animal in ['grom', 'jeev']:
        if run_alphas:
            ...
            if animal == 'grom':
                h5_name = '/Users/preeyakhanna/Dropbox/TimeMachineBackups/grom2016/grom_sweep_alpha_days_models_set%d.h5' %model_set_number
            elif animal == 'jeev':
                h5_name = '/Users/preeyakhanna/Dropbox/TimeMachineBackups/jeev2013/jeev_sweep_alpha_days_models_set%d.h5' %model_set_number
    ...
    pickle.dump(max_alphas, open('/Users/preeyakhanna/Dropbox/TimeMachineBackups/grom2016/max_alphas_ridge_model_set%d.pkl' %model_set_number, 'wb'))
