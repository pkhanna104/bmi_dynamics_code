        ...
        if plts: 
            ...
            for p in range(5):
                ...
                if fraction:
                    ...
                    print('Model: %s, X-pos %d, R2 mean %.3f, R2 norm %.3f'%(model_nms[p], p, 
                        np.mean(R2[model_nms[p]]), (np.mean(R2[model_nms[p]]) - r2_ref )/ r2_ref))
                else:
...
##### Fig S3 ########
def residual_PSTH(all_sessions = False): 
    ...
    if all_sessions: 
        ...
        ax[0].vlines(3., 0, 10, 'k', linestyle='dashed')
        ax[0].set_ylim([6., 9.])
        #ax[1].set_xlim([0, 2.2])
        return
